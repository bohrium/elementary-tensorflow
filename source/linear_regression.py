''' author: samtenka
    changed: 2017-10-06
    created: 2017-10-06
    descr: manually implement linear regression by stochastic gradient descent 
    usage: Run `python linear_regression.py`.
           Observe that loss decreases over time.  Observe the match between 
           true weights and the final values of the estimated weights. 
'''
...
# 2.4. We manually compute derivatives of the Loss with respect to relevant
#      nodes.  We use the chain rule to avoid repeating computation.  This use
#      of dynamic programming is called ``backpropagation''.  In practice, 
#      one relies on Tensorflow's ``automatic differentiation'' feature.  That
#      feature computes the derivatives implicitly when one trains neural
#      networks: 
